...
public class ApiParser {
    ...
    public static List<AvsItem> parse(InputStream stream, String boundary) throws IOException {
        ...
        if (mpStream.skipPreamble()) {
            ...
            // we have to use the count hack here because otherwise readBoundary() throws an
            // exception
            int count = 0;
        ...
        for (Directive directive : directives) {
            Log.i(TAG, "Parsing directive type: " + directive.getHeader().getNamespace() + ":"
            ...
            } else if (directive.isTypeSpeaker()) {
                item = new AvsSpeakerItem(directive.getPayload().getToken(), directive.getPayload());
                Log.e("SPEAKER ITEM HAS BEEN FOUND AND VOL IS",
